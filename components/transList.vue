      ...
      transactionsData() {
        ...
        this.transactionList.forEach((transaction) => {
          ...
          list.push({
            ...
            to: `<a href="${lang}/account/${transaction.to}" target="_blank" title="${transaction.to}">${transaction.shortTo}</a>${transaction.toAddressType === 1 ? `<span class="contract-tag">${this.$t("contract.commonContract")}</span>`: ""}`,
            from: `<a href="${lang}/account/${transaction.from}" target="_blank" title="${transaction.from}">${transaction.shortFrom}</a>${transaction.fromAddressType === 1 ? `<span class="contract-tag">${this.$t("contract.commonContract")}</span>`: ""}`,
